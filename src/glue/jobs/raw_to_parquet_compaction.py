...
def _resolve_compacted_path(compacted_bucket: str, compacted_prefix: str, ds: str) -> str:
    """Return the S3 URI where the compacted parquet partition should be stored."""

    normalized = _sanitize_prefix(compacted_prefix)
    return f"s3://{compacted_bucket}/{normalized}/ds={ds}"
...
def main() -> None:
    ...
    compacted_path = _resolve_compacted_path(args["compacted_bucket"], args["compacted_prefix"], args["ds"])
