        ...
        *,
        ...
        if domain_tables:
            for domain, table in domain_tables:
                ...
                raw_read_arns.append(iam_utils.bucket_objects_arn(raw_bucket.bucket_name, base_prefix))
                curated_read_arns.append(iam_utils.bucket_objects_arn(curated_bucket.bucket_name, base_prefix))
                curated_write_arns.append(iam_utils.bucket_objects_arn(curated_bucket.bucket_name, base_prefix))
                artifacts_schema_arns.append(
                    iam_utils.bucket_objects_arn(artifacts_bucket.bucket_name, f"{domain}/{table}/_schema")
                )
                ...
                for subdir in effective_subdirs:
                    ...
                    curated_write_arns.append(iam_utils.bucket_objects_arn(curated_bucket.bucket_name, prefix))
        else:
            raw_read_arns.append(iam_utils.bucket_objects_arn(raw_bucket.bucket_name))
            curated_read_arns.append(iam_utils.bucket_objects_arn(curated_bucket.bucket_name))
            curated_write_arns.append(iam_utils.bucket_objects_arn(curated_bucket.bucket_name))
            artifacts_schema_arns.append(iam_utils.bucket_objects_arn(artifacts_bucket.bucket_name, "_schema"))

        artifacts_temp_arn = iam_utils.bucket_objects_arn(artifacts_bucket.bucket_name, "temp")
        ...
        list_bucket_resources = iam_utils.dedupe(
            [iam_utils.bucket_arn(raw_bucket.bucket_name), iam_utils.bucket_arn(curated_bucket.bucket_name)]
        )
        artifacts_bucket_arn = iam_utils.bucket_arn(artifacts_bucket.bucket_name)
