...
├── {domain}/                         # 비즈니스 도메인
│   └── {table_name}/                 # 데이터셋/테이블
│       └── interval={interval}/      # 수집 해상도 (예: 1d, 1m)
...
- `interval`과 `data_source` 파티션 레벨을 추가하여 서로 다른 주기/소스 데이터를 안전하게 공존시키고, Glue/Preflight가 정확한 RAW 프리픽스를 계산할 수 있도록 합니다.
- 심볼은 파티션 키 대신 파일명으로 분리하여 하루치 재수집(동일 심볼·날짜)을 멱등하게 스킵할 수 있도록 설계했습니다.
...
| **3** | interval | 낮음 (15개) | ⭐⭐⭐⭐ | 매우 높음 | `1d` |
| **4** | data_source | 낮음 (5개) | ⭐⭐⭐ | 높음 | `yahoo_finance` |
| **5** | year | 높음 (연도) | ⭐⭐⭐⭐ | 높음 | `2025` |
| **6** | month | 중간 (12개) | ⭐⭐⭐⭐⭐ | 최고 | `09` |
| **7** | day | 높음 (31개) | ⭐⭐⭐⭐⭐ | 최고 | `22` |

> 심볼(Symbol)은 파티션 키에서 제외하고 개별 파일명으로 관리합니다.
...
|----------|-------------|:---------:|:-----------:|----------|
| **일별 데이터** | `year, month, day` | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | `WHERE year=2025 AND month=9 AND day=22` |
| **심볼별 분석** | 파일명 기준 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | `WHERE symbol='AAPL'` |
