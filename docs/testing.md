...
- 변경 사항 커밋 전:
  ...
  - 테스트가 있다면: `pytest -q`
- 커밋 후: 일반 커밋은 훅이 자동 실행됩니다. Black 또는 Ruff가 파일을 수정하면 재-`git add` 후 커밋하세요.
...
- Python 파일 변경 시
  ...
  - 포맷/린트: `black --line-length 120 --check`, `ruff check`
...
- 포함 훅
  ...
  - Ruff: 린트/자동수정(`ruff check --fix` 동등) 
  - mypy: `--ignore-missing-imports`
- 사용법
